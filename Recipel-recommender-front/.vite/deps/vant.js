...
var isSameValue = (newValue, oldValue) =>
    ...
    const style = computed(() =>
      mapThemeVarsToCSSVars(
        extend(
          ...
          props2.themeVars,
          props2.theme === "dark"
            ? props2.themeVarsDark
            : props2.themeVarsLight,
        ),
      ),
    );
    if (inBrowser) {
...
var stdin_default6 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    return () => {
      const { tag, dot, name: name210, size, badge, color } = props2;
        ...
        mergeProps(
          {
            ...
            class: [
              classPrefix.value,
              isImageIcon ? "" : `${classPrefix.value}-${name210}`,
            ],
            style: {
              color,
...
var stdin_default10 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    return () => {
      const { type, icon, text, color, loading, disabled } = props2;
      return createVNode(
        Button,
        {
          class: bem7([
  ...
  setup(props2, { emit, attrs, slots }) {
    ...
    const open = () => {
      if (!opened) {
        opened = true;
        zIndex.value =
          props2.zIndex !== void 0 ? +props2.zIndex : useGlobalZIndex();
        emit("open");
      }
    };
    const close = () => {
      ...
      if (props2.closeable) {
        return createVNode(
          Icon,
          {
            ...
            class: [
              bem10("close-icon", props2.closeIconPosition),
              HAPTICS_FEEDBACK,
            ],
            classPrefix: props2.iconPrefix,
    ...
    const renderTransition = () => {
      const { position, transition, transitionAppear } = props2;
      const name210 =
...
var getFirstEnabledOption = (options) =>
...
var isOptionExist = (options, value, fields) =>
  value !== void 0 &&
...
var stdin_default15 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const baseOffset = () =>
      (props2.optionHeight * (+props2.visibleOptionNum - 1)) / 2;
    ...
    const getIndexByOffset = (offset2) =>
      clamp(Math.round(-offset2 / props2.optionHeight), 0, count() - 1);
    ...
    const renderOptions = () => {
      ...
      return props2.options.map((option, index) => {
        ...
        return createVNode("li", data, [
          slots.option
            ? slots.option(option, index)
            : createVNode("div", childData, null),
        ]);
      });
    };
    ...
    watchEffect(() => {
      const index = moving
        ? Math.floor(-currentOffset.value / props2.optionHeight)
        : props2.options.findIndex(
            (option) => option[props2.fields.value] === props2.value,
          );
...
var stdin_default17 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const offset2 = computed(() =>
      unitToPx(
        props2.position === "top" ? props2.offsetTop : props2.offsetBottom,
      ),
    );
    const rootStyle = computed(() => {
      ...
      const { fixed, height: height2, width: width2 } = state;
      if (fixed) {
        return {
          width: `${width2}px`,
          height: `${height2}px`,
    ...
    const onScroll = () => {
      ...
      const { container, position } = props2;
      ...
      if (position === "top") {
        if (container) {
          ...
          const difference =
            containerRect.bottom - offset2.value - state.height;
          state.fixed =
            offset2.value > rootRect.top && containerRect.bottom > 0;
        ...
        if (container) {
          const containerRect = useRect(container);
          const difference =
            clientHeight - containerRect.top - offset2.value - state.height;
          state.fixed =
            clientHeight - offset2.value < rootRect.bottom &&
...
var stdin_default18 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const delta = computed(() =>
      props2.vertical ? touch.deltaY.value : touch.deltaX.value,
    );
    ...
    const move = ({ pace = 0, offset: offset2 = 0, emitChange }) => {
      if (count.value <= 1) {
        return;
      }
      const { active } = state;
    ...
    const onTouchStart = (event) => {
      if (
        !props2.touchable || // avoid resetting position on multi-finger touch
    ...
    watch(
      [windowWidth, windowHeight, () => props2.width, () => props2.height],
      resize,
    );
...
var stdin_default20 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const scrollable = computed(
      () =>
        children.length > +props2.swipeThreshold ||
        !props2.ellipsis ||
        props2.shrink,
    );
    ...
    const setLine = () => {
      ...
      nextTick(() => {
        ...
        const title = titles[state.currentIndex].$el;
        const { lineWidth, lineHeight } = props2;
    ...
    const setCurrentIndexByName = (name210, skipScrollIntoView) => {
      const matched = children.find(
        (tab, index2) => getTabName(tab, index2) === name210,
      );
    ...
    const getCurrentIndexOnScroll = () => {
      for (let index = 0; index < children.length; index++) {
        const { top: top2 } = useRect(children[index].$el);
    ...
    const renderHeader = () => {
      var _a, _b, _c;
      const { type, border, sticky } = props2;
      const Header = [
        createVNode(
          "div",
          {
    ...
    const resize = () => {
      ...
      nextTick(() => {
        var _a, _b;
        scrollIntoView(true);
        (_b = (_a = contentRef.value) == null ? void 0 : _a.swipeRef.value) ==
        null
          ? void 0
          : _b.resize();
      });
...
var stdin_default22 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    watchEffect(() => {
      const { titleClass, titleStyle } = props2;
    ...
    const renderTitle = (onClickTab) =>
      createVNode(
        ...
        mergeProps(
          ...
          pick(props2, ["dot", "badge", "title", "disabled", "showZeroBadge"]),
        ),
        {
          title: slots.title,
        },
...
var stdin_default23 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const showNextButton = () =>
      +activeTab.value < props2.tabs.length - 1 && props2.nextStepText;
    const onConfirm = () => {
      if (showNextButton()) {
              ...
              default: () => [
                props2.tabs.map((title, index) =>
                  createVNode(
                    ...
                    {
                      default: () => [
                        childNodes == null ? void 0 : childNodes[index],
                      ],
                    },
                  ),
                ),
...
var stdin_default24 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const fields = computed(() =>
      assignDefaultFields(props2.columnsFieldNames),
    );
    const optionHeight = computed(() => unitToPx(props2.optionHeight));
    const columnsType = computed(() =>
      getColumnsType(props2.columns, fields.value),
    );
      ...
      if (columnsType.value === "cascade") {
        selectedValues.value.forEach((value2, index) => {
          ...
          if (!isOptionExist(options, value2, fields.value)) {
            setValue(
              index,
              options.length ? options[0][fields.value.value] : void 0,
            );
          }
        });
      }
    ...
    const renderToolbar = () => {
      if (props2.showToolbar && !parent) {
        return createVNode(
          stdin_default16,
          mergeProps(pick(props2, pickerToolbarPropKeys), {
            onConfirm: confirm,
            onCancel: cancel,
          }),
          pick(slots, pickerToolbarSlots),
    ...
    return () => {
      ...
      return createVNode(
        ...
        [
          props2.toolbarPosition === "top" ? renderToolbar() : null,
          props2.loading
            ? createVNode(
                Loading,
                {
          ...
          props2.toolbarPosition === "bottom" ? renderToolbar() : null,
        ],
      );
    };
  },
  ...
  setup(props2, { emit, slots }) {
    ...
    useExpose({
      ...
      getSelectedOptions: () => {
        var _a;
        return (
          ((_a = picker.value) == null ? void 0 : _a.getSelectedOptions()) || []
        );
      },
    });
    return () =>
      createVNode(
        ...
        mergeProps(
          ...
          pick(props2, INHERIT_PROPS),
        ),
        pick(slots, INHERIT_SLOTS),
      );
  },
});
...
var stdin_default26 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    const renderTitle = () => {
      ...
      if (slots.title || isDef(props2.title)) {
        ...
        return createVNode(
          ...
          [
            titleSlot || createVNode("span", null, [props2.title]),
            renderLabel(),
          ],
        );
      }
    ...
    const renderRightIcon = () => {
      ...
      if (props2.isLink) {
        const name210 =
          props2.arrowDirection && props2.arrowDirection !== "right"
            ? `arrow-${props2.arrowDirection}`
            : "arrow";
        return createVNode(
          Icon,
          {
    ...
    return () => {
      var _a;
      const { tag, size, center, border, isLink, required } = props2;
    ...
    const submit = () => {
      ...
      validate()
        ...
        .catch((errors) => {
          ...
          const { scrollToError, scrollToErrorPosition } = props2;
  ...
  setup(props2, { emit, slots }) {
    ...
    const showClear = computed(() => {
      ...
      if (props2.clearable && !readonly) {
        ...
        const trigger =
          props2.clearTrigger === "always" ||
          (props2.clearTrigger === "focus" && state.focused);
    ...
    const showRequiredMark = computed(() => {
      ...
      if (required === "auto") {
        return (_a = props2.rules) == null
          ? void 0
          : _a.some((rule) => rule.required);
      }
      return required;
    });
    ...
    const validate = (rules = props2.rules) =>
      new Promise((resolve) => {
        resetValidation();
        if (rules) {
          emit("startValidate");
    ...
    const limitValueLength = (value) => {
      ...
      if (isDef(maxlength) && getStringLength(value) > +maxlength) {
        ...
        const selectionEnd =
          (_a = inputRef.value) == null ? void 0 : _a.selectionEnd;
    ...
    const updateValue = (value, trigger = "onChange") => {
      ...
      if (props2.type === "number" || props2.type === "digit") {
        ...
        if (
          trigger === "onBlur" &&
          value !== "" &&
          (props2.min !== void 0 || props2.max !== void 0)
        ) {
          const adjustedValue = clamp(
            +value,
            (_a = props2.min) != null ? _a : -Infinity,
            (_b = props2.max) != null ? _b : Infinity,
          );
      ...
      if (props2.formatter && trigger === props2.formatTrigger) {
        const { formatter, maxlength } = props2;
    ...
    const renderInput = () => {
      const controlClass = bem25("control", [
        getProp("inputAlign"),
        {
      ...
      return createVNode(
        "input",
        mergeProps(mapInputType(props2.type, props2.inputmode), inputAttrs),
        null,
      );
    };
    const renderLeftIcon = () => {
...
var stdin_default29 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const renderIcon = () => {
      const { icon, type, iconSize, iconPrefix, loadingType } = props2;
    ...
    watch(
      () => [props2.show, props2.type, props2.message, props2.duration],
      () => {
        clearTimer();
...
function createInstance() {
  const { instance: instance4, unmount } = mountComponent({
    setup() {
      const message = ref("");
...
var stdin_default30 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    return () => {
      var _a;
      const { size, loading, disabled, activeColor, inactiveColor } = props2;
      ...
      return createVNode(
        ...
        [
          ...
          (_a = slots.background) == null ? void 0 : _a.call(slots),
        ],
      );
    };
  },
...
var stdin_default31 = defineComponent({
  ...
  setup(props2, { emit }) {
    ...
    const showSearchResult = () =>
      props2.focused && props2.searchResult && props2.showSearchResult;
...
var stdin_default32 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const areaListLoaded = computed(
      () => isObject(props2.areaList) && Object.keys(props2.areaList).length,
    );
    ...
    const hideBottomFields = computed(() => {
      var _a;
      return (
        ((_a = props2.searchResult) == null ? void 0 : _a.length) &&
        detailFocused.value
      );
    });
    const onFocus = (key) => {
    ...
    const rules = computed(() => {
      const { validator, telValidator } = props2;
    ...
    watch(
      ...
      (value) => {
        ...
        nextTick(() => {
          var _a;
          const options =
            (_a = areaRef.value) == null ? void 0 : _a.getSelectedOptions();
          if (
            options &&
            options.every(
                ...
                [
                  ...
                  createVNode(
                    Field,
                    {
                      modelValue: data.tel,
                      "onUpdate:modelValue": [
                        ($event) => (data.tel = $event),
                        (val) => onChange("tel", val),
                      ],
                      clearable: true,
                      type: "tel",
                      label: t3("tel"),
...
var stdin_default35 = defineComponent({
  ...
  setup(props2, { slots, emit }) {
    ...
    const renderTag = () => {
      var _a;
      const { type, mark, plain, round: round2, size, closeable } = props2;
      const classes = {
        mark,
        plain,
        round: round2,
...
var stdin_default36 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const onClick = (event) => {
      ...
      const iconClicked =
        icon === target || (icon == null ? void 0 : icon.contains(target));
    ...
    const renderIcon = () => {
      var _a, _b;
      const { bem: bem107, checked, indeterminate } = props2;
          ...
          style:
            shape.value !== "dot"
              ...
              : {
                  ...
                  borderColor:
                    (_a = iconStyle.value) == null ? void 0 : _a.borderColor,
                },
        },
        [
          slots.icon
    ...
    const renderLabel = () => {
      const { checked } = props2;
      if (slots.default) {
        return createVNode(
          "span",
          {
            class: props2.bem("label", [
              props2.labelPosition,
              {
                disabled: disabled.value,
              },
            ]),
...
var stdin_default39 = defineComponent({
  ...
  setup(props2, { slots, emit }) {
    ...
    const renderContent = () => {
      const { address, disabled, switchable, singleChoice } = props2;
      const Info = [
        createVNode(
          "div",
          {
    ...
    return () => {
      ...
      return createVNode(
        ...
        [
          ...
          (_a = slots.bottom) == null
            ? void 0
            : _a.call(
                slots,
                extend({}, props2.address, {
                  disabled,
                }),
              ),
        ],
    ...
    return () => {
      ...
      return createVNode(
        ...
        [
          ...
          !singleChoice.value && Array.isArray(props2.modelValue)
            ? createVNode(
                CheckboxGroup,
                {
...
var getDPR = (scale = 1) =>
  inBrowser2 ? window.devicePixelRatio || scale : scale;
...
var stdin_default41 = defineComponent({
  ...
  setup(props2, { emit, slots, attrs }) {
    ...
    const scrollParent = ref();
    const style = computed(() =>
      extend(getZIndexStyle(props2.zIndex), {
    ...
    const scroll = () => {
      show.value = scrollParent.value
        ? getScrollTop(scrollParent.value) >= +props2.offset
        : false;
    };
    const getTarget = () => {
      const { target } = props2;
    ...
    const updateTarget = () => {
      if (inBrowser) {
        nextTick(() => {
          scrollParent.value = props2.target
            ? getTarget()
  ...
  setup(props2, { emit, slots }) {
    ...
    const appendBarrageItem = ({ id, text }, i) => {
      ...
      const item = createBarrageItem(
        text,
        isInitBarrage.value ? i * props2.delay : void 0,
      );
      ...
      total.value++;
      const top2 =
        ((total.value - 1) % +props2.rows) * item.offsetHeight + +props2.top;
    ...
    onMounted(() =>
      __async(this, null, function* () {
        ...
        rootStyle.value["--move-distance"] =
          `-${(_a = barrageWrapper.value) == null ? void 0 : _a.offsetWidth}px`;
        yield nextTick();
...
var formatMonthTitle = (date) =>
  t5("monthTitle", date.getFullYear(), date.getMonth() + 1);
...
var formatValueRange = (values, columns) =>
...
var stdin_default43 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    const style = computed(() => {
      const { item, index, color, offset: offset2, rowHeight } = props2;
      const style2 = {
        height: rowHeight,
      };
    ...
    const renderContent = () => {
      const { item, color, rowHeight } = props2;
      const { type } = item;
    ...
    return () => {
      const { type, className } = props2.item;
      if (type === "placeholder") {
        return createVNode(
          "div",
          {
...
var stdin_default44 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const totalDay = computed(() =>
      getMonthEndDay(props2.date.getFullYear(), props2.date.getMonth() + 1),
    );
    const shouldRender = computed(() => visible.value || !props2.lazyRender);
    const getTitle = () => title.value;
    const getMultipleDayType = (day) => {
      const isSelected = (date) =>
        props2.currentDate.some((item) => compareDay(item, date) === 0);
      if (isSelected(day)) {
    ...
    const getDayType = (day) => {
      const { type, minDate, maxDate, currentDate } = props2;
      if (
    ...
    const scrollToDate = (body, targetDate) => {
      if (daysRef.value) {
        ...
        const currentRow = Math.ceil(
          (targetDate.getDate() + offset2.value) / 7,
        );
  ...
  setup(props2, { slots, emit }) {
    const prevMonthDisabled = computed(
      () =>
        props2.date &&
        props2.minDate &&
        compareMonth(getPrevMonth(props2.date), props2.minDate) < 0,
    );
    const prevYearDisabled = computed(
      () =>
        props2.date &&
        props2.minDate &&
        compareMonth(getPrevYear(props2.date), props2.minDate) < 0,
    );
    const nextMonthDisabled = computed(
      () =>
        props2.date &&
        props2.maxDate &&
        compareMonth(getNextMonth(props2.date), props2.maxDate) > 0,
    );
    const nextYearDisabled = computed(
      () =>
        props2.date &&
        props2.maxDate &&
        compareMonth(getNextYear(props2.date), props2.maxDate) > 0,
    );
    const renderTitle = () => {
      if (props2.showTitle) {
    ...
    const renderAction = (isNext) => {
      ...
      const onMonthChange = () =>
        onPanelChange((isNext ? getNextMonth : getPrevMonth)(props2.date));
      const onYearChange = () =>
        onPanelChange((isNext ? getNextYear : getPrevYear)(props2.date));
      const MonthAction = createVNode(
        "view",
        {
    ...
    const renderWeekDays = () => {
      const { firstDayOfWeek } = props2;
      const weekdays = t5("weekdays");
      const renderWeekDays2 = [
...
var stdin_default46 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const getInitialDate = (defaultDate = props2.defaultDate) => {
      const { type, allowSameDay } = props2;
      if (defaultDate === null) {
        return defaultDate;
      }
      const now = getToday();
      if (type === "range") {
        ...
        const start2 = limitDateRange(
          defaultDate[0] || now,
          min,
          max ? (allowSameDay ? max : getPrevDay(max)) : void 0,
        );
        const end2 = limitDateRange(
          defaultDate[1] || (allowSameDay ? now : getNextDay(now)),
          min ? (allowSameDay ? min : getNextDay(min)) : void 0,
        );
        return [start2, end2];
      }
      if (type === "multiple") {
    ...
    const dayOffset = computed(() =>
      props2.firstDayOfWeek ? +props2.firstDayOfWeek % 7 : 0,
    );
    ...
    const scrollToCurrentDate = () => {
      ...
      if (currentDate.value) {
        const targetDate =
          props2.type === "single" ? currentDate.value : currentDate.value[0];
    ...
    const checkRange = (date) => {
      const { maxRange, rangePrompt, showRangePrompt } = props2;
    ...
    const onConfirm = () => {
      ...
      return emit(
        "confirm",
        (_a = currentDate.value) != null ? _a : cloneDates(currentDate.value),
      );
    };
    const select = (date, complete) => {
      ...
      if (complete && props2.type === "range") {
        ...
        if (!valid) {
          setCurrentDate([
            date[0],
            getDayByOffset(date[0], +props2.maxRange - 1),
          ]);
          return;
        }
      }
    ...
    const disabledDays = computed(() =>
      monthRefs.value.reduce((arr, ref2) => {
        var _a, _b;
        arr.push(
          ...((_b = (_a = ref2.disabledDays) == null ? void 0 : _a.value) !=
          null
            ? _b
            : []),
        );
        ...
        [
          createVNode(
            stdin_default45,
            {
              ...
              title: props2.title,
              subtitle:
                (_b = currentMonthRef.value) == null ? void 0 : _b.getTitle(),
    ...
    watch(
      () => [props2.type, props2.minDate, props2.maxDate, props2.switchMode],
  ...
  setup(props2, { emit, slots }) {
    ...
    const renderImage = () => {
      ...
      if (props2.lazyLoad) {
        return withDirectives(
          ...
          [[resolveDirective("lazy"), props2.src]],
        );
      }
      return createVNode(
        "img",
    ...
    onMounted(() => {
      nextTick(() => {
        var _a;
        if (
          ((_a = imageRef.value) == null ? void 0 : _a.complete) &&
          !props2.lazyLoad
        ) {
          triggerLoad();
        }
  ...
  setup(props2, { slots, emit }) {
    ...
    return () => {
      ...
      const showOriginPrice =
        slots["origin-price"] || isDef(props2.originPrice);
      const showBottom =
      ...
      const OriginPrice =
        ...
        createVNode(
          ...
          [
            slots["origin-price"]
              ? slots["origin-price"]()
              : `${props2.currency} ${props2.originPrice}`,
          ],
        );
      const Num =
        showNum &&
...
var stdin_default49 = defineComponent({
  ...
  setup(props2, { slots, emit }) {
    ...
    const updateTabs = () => {
      const { options, modelValue } = props2;
    ...
    const onClickTab = ({ name: name210, title }) =>
      ...
      return createVNode(
        ...
        },
        {
          default: () => {
            var _a, _b;
            return [
              (_a = slots["options-top"]) == null
                ? void 0
                : _a.call(slots, {
                    tabIndex,
                  }),
              renderOptions(options, selected, tabIndex),
              (_b = slots["options-bottom"]) == null
                ? void 0
                : _b.call(slots, {
                    tabIndex,
                  }),
...
var stdin_default51 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    watch(
      ...
      (rate) => {
        ...
        const duration = Math.abs(
          ((startRate - endRate) * 1e3) / +props2.speed,
        );
        const animate = () => {
          const now = Date.now();
    ...
    const renderHover = () => {
      ...
      const { strokeWidth, currentRate, strokeLinecap } = props2;
      ...
      createVNode(
        ...
        [
          createVNode(
            ...
            [renderGradient(), renderLayer(), renderHover()],
          ),
          renderText(),
        ],
      );
...
var stdin_default52 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    return () => {
      const { tag, wrap, align, justify } = props2;
      return createVNode(
        tag,
        {
          class: bem45({
  ...
  setup(props2, { slots }) {
    ...
    const style = computed(() => {
      ...
      if (spaces && spaces.value && spaces.value[index.value]) {
        const { left: left2, right: right2 } = spaces.value[index.value];
        styles = {
          paddingLeft: left2 ? `${left2}px` : null,
          paddingRight: right2 ? `${right2}px` : null,
        };
      }
      const { bottom: bottom2 } = verticalSpaces.value[index.value] || {};
      return extend(styles, {
        marginBottom: bottom2 ? `${bottom2}px` : null,
      });
    });
    return () => {
      const { tag, span, offset: offset2 } = props2;
      return createVNode(
        tag,
        {
          style: style.value,
...
var stdin_default55 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    const renderTitle = () => {
      const { border, disabled, readonly } = props2;
...
var stdin_default56 = defineComponent({
  ...
  setup(props2, { emit }) {
    ...
    const renderContent = () => {
      ...
      return [
        createVNode("div", null, [`${t7("name")}：${props2.name}`]),
        createVNode("div", null, [`${t7("tel")}：${props2.tel}`]),
      ];
    };
    return () =>
      createVNode(
...
var stdin_default60 = defineComponent({
  ...
  setup(props2) {
    const validPeriod = computed(() => {
      const { startAt, endAt } = props2.coupon;
      return `${getDate(startAt)} - ${getDate(endAt)}`;
    });
    const faceAmount = computed(() => {
      const { coupon, currency } = props2;
      if (coupon.valueDesc) {
        return [
          coupon.valueDesc,
          createVNode("span", null, [coupon.unitDesc || ""]),
        ];
      }
    ...
    return () => {
      const { chosen, coupon, disabled } = props2;
...
var stdin_default61 = defineComponent({
  ...
  setup(props2) {
    return () => {
      const selected = Array.isArray(props2.chosenCoupon)
        ? props2.chosenCoupon.length
        : props2.coupons[+props2.chosenCoupon];
      return createVNode(
        Cell,
        {
          class: bem54(),
...
var stdin_default62 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    const renderStop = (color, offset2, opacity) =>
      createVNode(
        "stop",
        {
          "stop-color": color,
    ...
    const renderStops = (fromColor, toColor) => [
      renderStop(fromColor, 0),
      renderStop(toColor, 100),
    ];
    const renderShadow = (id) => [
      createVNode("defs", null, [
        createVNode(
          ...
          [renderStop("#EBEDF0", 0), renderStop("#F2F3F5", 100, 0.3)],
        ),
      ]),
      createVNode(
        "ellipse",
    ...
    const renderBuilding = () => [
      createVNode("defs", null, [
        createVNode(
          ...
          [renderStop("#FFF", 0, 0.5), renderStop("#F2F3F5", 100)],
        ),
      ]),
      createVNode(
        "g",
    ...
    const renderCloud = () => [
      createVNode("defs", null, [
        createVNode(
          ...
          [renderStop("#F2F3F5", 0, 0.3), renderStop("#F2F3F5", 100)],
        ),
      ]),
      createVNode(
        "g",
    ...
    const renderImage = () => {
      ...
      return (
        ((_a = PRESET_IMAGES[props2.image]) == null
          ? void 0
          : _a.call(PRESET_IMAGES)) ||
        createVNode(
          "img",
          {
            src: props2.image,
...
var stdin_default63 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const buttonDisabled = computed(
      () =>
        !props2.exchangeButtonLoading &&
        (props2.exchangeButtonDisabled ||
          !currentCode.value ||
          currentCode.value.length < props2.exchangeMinLength),
    );
    ...
    const scrollToCoupon = (index) => {
      nextTick(() => {
        var _a;
        return (_a = couponRefs.value[index]) == null
          ? void 0
          : _a.scrollIntoView();
      });
    };
    ...
    const renderCouponTab = () => {
      const { coupons, chosenCoupon } = props2;
                  ...
                  coupons.map((coupon, index) =>
                    createVNode(
                      Coupon,
                      {
                        ...
                        coupon: coupon,
                        chosen: Array.isArray(chosenCoupon)
            ...
            return [
              createVNode(
                ...
                [
                  ...
                  (_a = slots["disabled-list-footer"]) == null
                    ? void 0
                    : _a.call(slots),
                ],
              ),
                    ...
                    createVNode(
                      Button,
                      {
                        ...
                        onClick: () =>
                          emit(
                            "change",
                            Array.isArray(props2.chosenCoupon) ? [] : -1,
                          ),
                      },
                      null,
                    ),
                    [[vShow, props2.showCloseButton]],
                  ),
            ],
          ),
        ],
...
var stdin_default64 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const computedValues = computed(() =>
      updatedByExternalSources.value ? props2.modelValue : currentValues.value,
    );
    ...
    const getValue2 = (type) => {
      const { minDate, columnsType } = props2;
    ...
    const genDayOptions = () => {
      ...
      const minDate =
        isMinYear(year) && isMinMonth(month) ? props2.minDate.getDate() : 1;
      const maxDate =
        isMaxYear(year) && isMaxMonth(month)
          ? props2.maxDate.getDate()
  ...
  setup(props2, { emit, slots }) {
    ...
    const onKeydown = withKeys(
      (event) => {
        ...
        if (
          event.target !==
          ((_b = (_a = root.value) == null ? void 0 : _a.popupRef) == null
            ? void 0
            : _b.value)
        ) {
          return;
    ...
    const renderMessage = (hasTitle) => {
      const { message, allowHtml, messageAlign } = props2;
    ...
    const renderContent = () => {
      ...
      const { title, message, allowHtml } = props2;
      if (message) {
    ...
    const renderFooter = () => {
      ...
      return props2.theme === "round-button"
        ? renderRoundButtons()
        : renderButtons();
    };
    return () => {
      const { width: width2, title, theme, message, className } = props2;
      return createVNode(
        Popup,
        mergeProps(
          {
      ...
      return createVNode(
        "div",
        {
          ...
          class: bem58({
            ...
            vertical: props2.vertical,
            [`content-${props2.contentPosition}`]:
              !!slots.default && !props2.vertical,
          }),
        },
        [
          !props2.vertical &&
...
var stdin_default67 = defineComponent({
  ...
  setup(props2, { slots }) {
    ...
    const scrollable = computed(
      () => props2.swipeThreshold && children.length > +props2.swipeThreshold,
    );
...
var stdin_default68 = defineComponent({
  ...
  setup(props2, { emit, slots, attrs }) {
    ...
    const renderTitle = () => {
      ...
      const match = props2.options.find(
        (option) => option.value === props2.modelValue,
      );
            ...
            createVNode(
              ...
              },
              {
                default: () => {
                  var _a;
                  return [
                    props2.options.map(renderOption),
                    (_a = slots.default) == null ? void 0 : _a.call(slots),
                  ];
                },
              },
...
var stdin_default69 = defineComponent({
  ...
  setup(props2, { slots, emit, attrs }) {
    ...
    const updateState = () => {
      ...
      const { width: width2, height: height2 } = useRect(rootRef.value);
      const { offset: offset2 } = props2;
      state.value = {
        x: offset2.x > -1 ? offset2.x : windowWidth.value - width2 - props2.gap,
        y:
          offset2.y > -1
            ? offset2.y
            : windowHeight.value - height2 - props2.gap,
        width: width2,
        height: height2,
      };
    };
    ...
    watch(
      [windowWidth, windowHeight, () => props2.gap, () => props2.offset],
      updateState,
      {
        deep: true,
      },
  ...
  setup(props2, { emit, slots }) {
    ...
    const boundary = computed(() => {
      ...
      return {
        ...
        max:
          (_b = props2.anchors[props2.anchors.length - 1]) != null
            ? _b
            : Math.round(windowHeight.value * 0.6),
      };
    });
    const anchors = computed(() =>
      props2.anchors.length >= 2
        ? props2.anchors
    ...
    const rootStyle = computed(() => ({
      ...
      transition: !dragging.value
        ? `transform ${props2.duration}s cubic-bezier(0.18, 0.89, 0.32, 1.28)`
        : "none",
    }));
    const ease = (moveY) => {
    ...
    const onTouchmove = (e) => {
      ...
      if (
        ...
        ((_a = contentRef.value) == null ? void 0 : _a.contains(target))
      ) {
...
var stdin_default73 = defineComponent({
  ...
  setup(props2) {
    const highlightChunks = computed(() => {
      const { autoEscape, caseSensitive, keywords, sourceString } = props2;
    ...
    const renderContent = () => {
      ...
      return highlightChunks.value.map((chunk) => {
        const { start: start2, end: end2, highlight } = chunk;
...
var stdin_default74 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const maxMoveX = computed(() => {
      if (state.imageRatio) {
        const { rootWidth, rootHeight } = props2;
    ...
    const setScale = (scale, center) => {
      ...
      if (scale !== state.scale) {
        ...
        state.scale = scale;
        if (center) {
          const imageRect = useRect(
            (_a = imageRef.value) == null ? void 0 : _a.$el,
          );
          const origin = {
    ...
    const checkClose = (event) => {
      ...
      const isClickImage =
        imageEl == null ? void 0 : imageEl.contains(event.target);
...
var stdin_default75 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
      ...
      if (props2.closeable) {
        return createVNode(
          Icon,
          {
            ...
            name: props2.closeIcon,
            class: [
              bem67("close-icon", props2.closeIconPosition),
              HAPTICS_FEEDBACK,
            ],
            onClick: emitClose,
          },
    ...
    const swipeTo = (index, options) => {
      var _a;
      return (_a = swipeRef.value) == null
        ? void 0
        : _a.swipeTo(index, options);
    };
    useExpose({
      resetScale: () => {
        var _a;
    ...
    watch(
      () => props2.startPosition,
      (value) => setActive(+value),
    );
    watch(
      () => props2.show,
      (value) => {
        const { images, startPosition } = props2;
        if (value) {
...
function initInstance2() {
  ({ instance: instance2 } = mountComponent({
    setup() {
      const { state, toggle } = usePopupState();
      const onClosed = () => {
        state.images = [];
  ...
  setup(props2, { emit, slots }) {
    ...
    const getActiveAnchor = (scrollTop, rects) => {
      for (let i = children.length - 1; i >= 0; i--) {
        ...
        const reachTop = props2.sticky
          ? prevHeight + props2.stickyOffsetTop
          : 0;
    ...
    const getMatchAnchor = (index) =>
      children.find((item) => String(item.index) === index);
    const onScroll = () => {
      if (isHidden(root)) {
        return;
      }
      const { sticky, indexList } = props2;
      ...
      if (sticky) {
        children.forEach((item, index) => {
          ...
          if (index === active) {
            state.active = true;
            state.top =
              Math.max(props2.stickyOffsetTop, rects[index].top - scrollTop) +
  ...
  setup(props2, { emit, slots }) {
    ...
    const check = () => {
      nextTick(() => {
        if (
          ...
          props2.finished ||
          props2.disabled ||
          props2.error || // skip check when inside an inactive tab
        ...
        if (direction === "up") {
          ...
          isReachEdge =
            placeholderRect.bottom - scrollParentRect.bottom <= offset2;
        }
        if (isReachEdge) {
          loading.value = true;
    ...
    return () => {
      ...
      return createVNode(
        ...
        [
          props2.direction === "down" ? Content : Placeholder,
          renderLoading(),
          renderFinishedText(),
          renderErrorText(),
          props2.direction === "up" ? Content : Placeholder,
        ],
      );
    };
  },
...
var stdin_default79 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const renderNavBar = () => {
      const { title, fixed, border, zIndex } = props2;
      const style = getZIndexStyle(zIndex);
                ...
                createVNode(
                  "div",
                  {
                    class: [
                      ...
                      props2.clickable && !props2.leftDisabled
                        ? HAPTICS_FEEDBACK
                        : "",
                    ],
                    onClick: onClickLeft,
                ...
                createVNode(
                  "div",
                  {
                    class: [
                      ...
                      props2.clickable && !props2.rightDisabled
                        ? HAPTICS_FEEDBACK
                        : "",
                    ],
...
var stdin_default80 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const reset = () => {
      const { delay, speed, scrollable } = props2;
    ...
    return () => {
      const { color, wrapable, background } = props2;
      return withDirectives(
        createVNode(
          "div",
          {
...
function initInstance3() {
  ({ instance: instance3 } = mountComponent({
    setup() {
      const { state, toggle } = usePopupState();
      return () =>
        createVNode(
...
var stdin_default83 = defineComponent({
  ...
  setup(props2, { emit, slots, attrs }) {
    ...
    const keys = computed(() =>
      props2.theme === "custom" ? genCustomKeys() : genDefaultKeys(),
    );
    const onBlur = () => {
      if (props2.show) {
        emit("blur");
    ...
    const renderTitle = () => {
      const { title, theme, closeButtonText } = props2;
...
var stdin_default84 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    const count = computed(() => {
      const { pageCount, totalItems, itemsPerPage } = props2;
    ...
    const pages = computed(() => {
      ...
      const { modelValue, forceEllipses } = props2;
      let startPage = 1;
    ...
    const renderDesc = () =>
      createVNode(
        ...
        [
          slots.pageDesc
            ? slots.pageDesc()
            : `${props2.modelValue}/${count.value}`,
        ],
      );
    const renderPrevButton = () => {
      const { mode, modelValue, showPrevButton } = props2;
      if (!showPrevButton) {
        return;
      }
    ...
    const renderNextButton = () => {
      const { mode, modelValue, showNextButton } = props2;
      if (!showNextButton) {
        return;
      }
      ...
      createVNode(
        ...
        [
          createVNode(
            ...
            [
              renderPrevButton(),
              props2.mode === "simple" ? renderDesc() : renderPages(),
              renderNextButton(),
            ],
          ),
        ],
...
var stdin_default85 = defineComponent({
  ...
  setup(props2, { emit }) {
    ...
    const renderPoints = () => {
      const Points = [];
      const { mask, value, gutter, focused } = props2;
...
function getBoundingClientRect(element, includeScale, isFixedStrategy) {
  ...
  var _ref = isElement2(element) ? getWindow(element) : window,
...
function getDocumentElement(element) {
  return (
    (isElement2(element) ? element.ownerDocument : element.document) ||
    window.document
  ).documentElement;
}
  ...
  return function createPopper2(reference, popper, options) {
    ...
    var instance4 = {
      ...
      setOptions: function setOptions(setOptionsAction) {
        ...
        state.scrollParents = {
          reference: isElement2(reference)
      ...
      forceUpdate: function forceUpdate() {
        ...
        for (var index = 0; index < state.orderedModifiers.length; index++) {
          ...
          var _state$orderedModifie = state.orderedModifiers[index],
            ...
            name112 = _state$orderedModifie.name;
  ...
  return function () {
    Object.keys(state.elements).forEach(function (name112) {
      ...
      var styleProperties = Object.keys(
        state.styles.hasOwnProperty(name112)
          ? state.styles[name112]
          : initialStyles[name112],
      );
...
var stdin_default86 = defineComponent({
  ...
  setup(props2, { emit, slots, attrs }) {
    ...
    const onClickAway = () => {
      if (
        show.value &&
        props2.closeOnClickOutside &&
        (!props2.overlay || props2.closeOnClickOverlay)
      ) {
        show.value = false;
      }
    };
    ...
    onMounted(() => {
      ...
      watchEffect(() => {
        var _a;
        popupRef.value =
          (_a = popoverRef.value) == null ? void 0 : _a.popupRef.value;
      });
    });
    onBeforeUnmount(() => {
      if (popper) {
...
var stdin_default87 = defineComponent({
  ...
  setup(props2) {
    const background = computed(() =>
      props2.inactive ? void 0 : props2.color,
    );
    const renderPivot = () => {
      const { textColor, pivotText, pivotColor, percentage } = props2;
    ...
    return () => {
      const { trackColor, percentage, strokeWidth } = props2;
      const rootStyle = {
...
var stdin_default90 = defineComponent({
  ...
  setup(props2) {
    const newFigureArr = computed(() =>
      props2.direction === "down"
        ? props2.figureArr.slice().reverse()
        : props2.figureArr,
    );
...
var stdin_default91 = defineComponent({
  ...
  setup(props2) {
    const isCustomType = computed(
      () => Array.isArray(props2.textList) && props2.textList.length,
    );
    ...
    const startNumArr = computed(() =>
      padZero(props2.startNum, itemLength.value).split(""),
    );
...
var stdin_default93 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const renderOption = (option, index) => {
      const { name: name210, icon, className, description } = option;
      return createVNode(
        "div",
        {
          role: "button",
...
var stdin_default95 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    return () => {
      const { dot, badge, title, disabled } = props2;
...
var stdin_default96 = defineComponent({
  ...
  setup(props2, { emit }) {
    ...
    const touchMove = (event) => {
      ...
      const touch = event.touches[0];
      const mouseX =
        touch.clientX - ((canvasRect == null ? void 0 : canvasRect.left) || 0);
      const mouseY =
        touch.clientY - ((canvasRect == null ? void 0 : canvasRect.top) || 0);
    ...
    const submit = () => {
      ...
      const image = isEmpty
        ...
        : ((_b = (_a = {
            ...
            ? void 0
            : _b.call(_a)) || canvas.toDataURL(`image/${props2.type}`);
      emit("submit", {
        image,
        canvas,
      });
    ...
    const initialize = () => {
      ...
      if (isRenderCanvas && canvasRef.value) {
        ...
        canvasWidth = canvas.width =
          (((_a = wrapRef.value) == null ? void 0 : _a.offsetWidth) || 0) * dpr;
        canvasHeight = canvas.height =
          (((_b = wrapRef.value) == null ? void 0 : _b.offsetHeight) || 0) *
          dpr;
          ...
          createVNode(
            ...
            [
              isRenderCanvas
                ...
                : createVNode("p", null, [props2.tips]),
            ],
          ),
          createVNode(
            "div",
...
var stdin_default105 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const onClick = (event) => {
      ...
      updateStartValue();
      const { min, reverse, vertical, modelValue } = props2;
...
function filterEmpty(children = []) {
  ...
  return nodes.filter((c) => {
    ...
    return !(
      ...
      (c.type === Comment ||
        (c.type === Fragment &&
          ((_a = c.children) == null ? void 0 : _a.length) === 0) ||
...
var stdin_default106 = defineComponent({
  ...
  setup(props2, { slots }) {
    const mergedAlign = computed(() => {
      var _a;
      return (_a = props2.align) != null
        ? _a
        : props2.direction === "horizontal"
          ? "center"
          : "";
    });
...
var stdin_default109 = defineComponent({
  ...
  setup(props2, { emit }) {
    const format3 = (value, autoFixed = true) => {
      const { min, max, allowEmpty, decimalLength } = props2;
      if (allowEmpty && value === "") {
        return value;
      }
    ...
    const getInitialValue = () => {
      var _a;
      const defaultValue =
        (_a = props2.modelValue) != null ? _a : props2.defaultValue;
    ...
    const minusDisabled = computed(
      () =>
        props2.disabled ||
        props2.disableMinus ||
        +current2.value <= +props2.min,
    );
    const plusDisabled = computed(
      () =>
        props2.disabled || props2.disablePlus || +current2.value >= +props2.max,
    );
    ...
    watch(
      () => [props2.max, props2.min, props2.integer, props2.decimalLength],
      check,
    );
    watch(
      () => props2.modelValue,
    ...
    const renderText = () => {
      ...
      if (typeof price === "number") {
        ...
        return createVNode(
          ...
          [
            createVNode("span", null, [label || t20("label")]),
            createVNode(
              "span",
              {
                class: bem99("price"),
...
var stdin_default111 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const leftWidth = computed(() =>
      isDef(props2.leftWidth) ? +props2.leftWidth : getWidthByRef(leftRef),
    );
    const rightWidth = computed(() =>
      isDef(props2.rightWidth) ? +props2.rightWidth : getWidthByRef(rightRef),
    );
    const open = (side) => {
...
var stdin_default112 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const renderTabbar = () => {
      var _a;
      const { fixed, zIndex, border } = props2;
      return createVNode(
        "div",
        {
          ref: root,
...
var stdin_default113 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const active = computed(() => {
      var _a;
      const { route: route22, modelValue } = parent.props;
      if (route22 && "$route" in vm) {
        const { $route } = vm;
        const { to } = props2;
        const config = isObject(to)
          ? to
          : {
              path: to,
            };
...
var stdin_default114 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const cloneContainer = () => {
      ...
      styleNames.forEach((name210) => {
        container.style.setProperty(
          name210,
          originStyle.getPropertyValue(name210),
        );
      });
    ...
    const calcEllipsisText = (container, maxHeight) => {
      var _a, _b;
      const { content, position, dots } = props2;
      const end2 = content.length;
      const middle = (0 + end2) >> 1;
      const actionHTML = slots.action
        ? (_b = (_a = actionRef.value) == null ? void 0 : _a.outerHTML) != null
          ? _b
          : ""
        : props2.expandText;
      const calcEllipse = () => {
      ...
      const middleTail = (leftPart, rightPart) => {
        if (
          ...
          return (
            ...
            dots +
            content.slice(rightPart[1], end2)
          );
        }
        const leftMiddle = Math.floor((leftPart[0] + leftPart[1]) / 2);
        const rightMiddle = Math.ceil((rightPart[0] + rightPart[1]) / 2);
        container.innerText =
          props2.content.slice(0, leftMiddle) +
          props2.dots +
          props2.content.slice(rightMiddle, end2);
        container.innerHTML += actionHTML;
        if (container.offsetHeight >= maxHeight) {
          return middleTail(
      ...
      return props2.position === "middle"
        ? middleTail([0, middle], [middle, end2])
        : calcEllipse();
    };
    const calcEllipsised = () => {
      ...
      const maxHeight = Math.ceil(
        (Number(props2.rows) + 0.5) * pxToNum(lineHeight) +
          pxToNum(paddingTop) +
    ...
    watch(
      [windowWidth, () => [props2.content, props2.rows, props2.position]],
      calcEllipsised,
    );
    useExpose({
      toggle,
    ...
    return () =>
      createVNode(
        ...
        [
          expanded.value ? props2.content : text.value,
  ...
  setup(props2, { emit, slots }) {
    ...
    const getValidTime = (time) => {
      ...
      return fullColumns.map((col, i) =>
        props2.columnsType.includes(col) ? timeLimitArr[i] : "00",
      );
    };
    const confirm = () => {
      var _a;
...
var stdin_default116 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    const isActiveItem = (id) =>
      Array.isArray(props2.activeId)
        ? props2.activeId.includes(id)
        : props2.activeId === id;
    ...
    const renderSidebar = () => {
      const Items = props2.items.map((item) =>
        createVNode(
          SidebarItem,
          {
            dot: item.dot,
...
var stdin_default117 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    const renderMask = () => {
      const { status, message } = props2.item;
    ...
    const onDelete = (event) => {
      const { name: name112, item, index, beforeDelete } = props2;
      event.stopPropagation();
    ...
    const renderPreview = () => {
      const { item, lazyLoad, imageFit, previewSize, reupload } = props2;
      if (isImageFile(item)) {
        return createVNode(
          Image2,
          {
...
var stdin_default118 = defineComponent({
  ...
  setup(props2, { emit, slots }) {
    ...
    const readFile = (files) => {
      const { maxCount, modelValue, resultType } = props2;
        ...
        mergeProps(
          {
            ...
            onClick: () =>
              emit(
                props2.reupload ? "clickReupload" : "clickPreview",
                item,
                getDetail(index),
              ),
          ...
          pick(props2, ["name", "lazyLoad"]),
          previewData,
        ),
        pick(slots, ["preview-cover", "preview-delete"]),
      );
    };
    ...
    const renderUpload = () => {
      ...
      return withDirectives(
        ...
        [[vShow, props2.showUpload && lessThanMax]],
      );
    };
    const chooseFile = () => {
...
var ReactiveListener = class {
  ...
  load(onFinish = noop) {
    ...
    this.renderLoading(() => {
      var _a, _b;
      this.attempt++;
      (_b = (_a = this.options.adapter).beforeLoad) == null
        ? void 0
        : _b.call(_a, this, this.options);
  ...
  performance() {
    ...
    if (this.state.loaded) {
      state = "loaded";
      time =
        (this.performanceData.loadEnd - this.performanceData.loadStart) / 1e3;
    }
  ...
  return class Lazy {
    ...
    initListen(el, start2) {
      this.options.ListenEvents.forEach((evt) =>
        (start2 ? on : off)(el, evt, this.lazyLoadHandler),
      );
    }
    initEvent() {
      this.Event = {
    ...
    imgs.forEach((el2) => {
      this.lazy.add(
        ...
        Object.assign({}, this.binding, {
          value: {
            src:
              "dataset" in el2 ? el2.dataset.src : el2.getAttribute("data-src"),
            error:
              ("dataset" in el2
                ? el2.dataset.error
                : el2.getAttribute("data-error")) || this.options.error,
            loading:
              ("dataset" in el2
                ? el2.dataset.loading
                : el2.getAttribute("data-loading")) || this.options.loading,
          },
        }),
        this.vnode,
      );
